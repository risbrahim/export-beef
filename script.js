...
function genBatches() {
  ...
  const statuses = ['Di Farm','Dalam Proses','Cold Storage','Distribusi','Export Hub','Terjual'];
...
const MASTER = {
  ...
  export hub: {
    ...
    rows:[
      ['R001','Supermarket Chain A','Export Market','45','Aktif'],
      ['R002','Pasar Modern B','Export Market','32','Aktif'],
...
const TIMELINE = [
  ...
  { stage:'Export Hub / Pasar', lokasi:'Supermarket & Pasar Modern', tanggal:'10/02/2024', durasi:'—', status:'waiting', catatan:'—' },
];
...
const pageMeta = {
  ...
  traceability:{ title:'Sistem Digital Traceability',  sub:'Lacak perjalanan setiap batch dari farm hingga konsumen' },
  forecasting: { title:'Demand Demand Forecasting',   sub:'Prediksi permintaan dengan berbagai metode statistik' },
  ...
  masterdata:  { title:'Data Master',          sub:'Manajemen entitas farm, distributor, dan export hub' },
};
...
function initPage(pageId) {
  switch(pageId) {
    ...
    case 'traceability': initDigital Traceability(); break;
    case 'forecasting':  initDemand Forecasting();  break;
...
function initDashboard() {
  ...
  // Channel doughnut
  const channelLabels  = ['Export Market','HoReCa','Traditional','Export Distribution','Lainnya'];
  ...
  const statusCls = { 'Di Farm':'badge-blue','Dalam Proses':'badge-amber','Cold Storage':'badge-purple','Distribusi':'badge-amber','Export Hub':'badge-green','Terjual':'badge-green' };
...
═══════════════════════════ */
let _batches = [];
function initDigital Traceability() {
  _batches = genBatches();
...
/* ═══════════════════════════
   FORECASTING
═══════════════════════════ */
function initDemand Forecasting() {
  updateDemand Forecasting();
  renderAccuracy();
}

function updateDemand Forecasting() {
...
const scNodes = [
  ...
  { label:'Supermarket\nChain A',       type:'export hub',    cap:3000, cur:2100 },
  { label:'Pasar Modern\nB',            type:'export hub',    cap:2500, cur:1800 },
  { label:'HoReCa\nJakarta',            type:'export hub',    cap:4000, cur:3200 },
];
const typeColor = { farm:'var(--primary)', processing:'var(--blue)', storage:'var(--purple)', distributor:'var(--accent)', export hub:'var(--success)' };
...
function initSupplyChain() {
  ...
  // Flow columns
  const cols = { farm:[], processing:[], storage:[], distributor:[], export hub:[] };
  scNodes.forEach(n => cols[n.type].push(n));
  const labels = { farm:'🌾 Farm', processing:'🔪 RPH', storage:'❄️ Cold Storage', distributor:'🚛 Distribusi', export hub:'🏪 Export Hub' };
...
/* ═══════════════════════════
   MASTER DATA
═══════════════════════════ */
const masterTitles = { farm:'🌾 Data Farm', distributor:'🚛 Data Distributor', export hub:'🏪 Data Export Hub' };
